//+------------------------------------------------------------------+
//|                               IFVG_DualModel_EA_Aggressive.mq5  |
//|                                                     txatxe.code  |
//|                                                                  |
//| VARIANTE AGRESIVA del IFVG_DualModel_EA. Misma logica de entrada |
//| (Modelo 1: M15->M5->IFVG / Modelo 2: sweep->displacement->IFVG), |
//| con los parametros de la variante agresiva documentada en los    |
//| backtests de ict_macro_bot (2026-06-27):                         |
//|                                                                  |
//|   focus_sl150_trail75_50_minrange125_maxloss3                    |
//|   -> min range 125 (vs 175 del recomendado)                      |
//|   -> PnL +1792.99 (+448.25%) pero drawdown 342.56 (vs 89.92)     |
//|                                                                  |
//| ATENCION: mas operaciones y mas beneficio en backtest a cambio   |
//| de casi 4x mas drawdown. No usar en cuenta live sin asumirlo.    |
//+------------------------------------------------------------------+
...
//--- inputs generales
input group "=== General ==="
input long              InpMagic            = 20260709;   // Magic number (distinto de la version normal)
...
input int               InpM2MinRangePoints = 125;        // Rango minimo reciente (variante agresiva: 125)
...
int OnInit()
  {
   ...
   Print("IFVG_DualModel_EA_Aggressive iniciado. Modo=", EnumToString(InpModelMode),
         " Simbolo=", _Symbol);
...
bool TryOpen(const ENUM_ORDER_TYPE type, const int model)
  {
   ...
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double sl, tp = 0.0;
   string comment = (model == 1) ? "IFVG_AGG_M1" : "IFVG_AGG_M2";
...
void UpdatePanel()
  {
   ...
   string txt = StringFormat(
      "IFVG DualModel EA AGGRESSIVE | Modo: %s\n"

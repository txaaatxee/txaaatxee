//+------------------------------------------------------------------+
//|                                 IFVG_DualModel_EA_Axi_Live.mq5  |
//|                                                     txatxe.code  |
//|                                                                  |
...
//|     arrancar como referencia del escalado de lote                |
//|   - Aviso en el log si el simbolo no cotiza con punto=$0.01      |
//+------------------------------------------------------------------+
...
input int               InpM2FvgMinPoints   = 500;        // Tamano minimo del FVG (puntos, Axi = $5)
input int               InpM2MinRangePoints = 17500;      // Rango minimo reciente (puntos, Axi = $175)
...
input double            InpInitialBalance   = 0.0;        // Balance inicial (0 = usar balance actual al arrancar)
...
input int               InpStopPoints       = 15000;      // Stop loss fijo (puntos, Axi = $150)
...
input int               InpTrailStartPoints = 7500;       // Trailing start (puntos, Axi = $75)
input int               InpTrailDistPoints  = 5000;       // Trailing distance (puntos, Axi = $50)
...
// balance de referencia para el escalado de lote
...
int OnInit()
  {
   ...
   // balance de referencia: input fijo o balance actual de la cuenta
   g_initialBalance = (InpInitialBalance > 0.0)
                      ? InpInitialBalance
                      : AccountInfoDouble(ACCOUNT_BALANCE);

   // los defaults estan calibrados para punto = 0.01 (Axi BTCUSD, 2 digitos)
   if(MathAbs(_Point - 0.01) > 1e-10)
      PrintFormat("AVISO: %s cotiza con punto=%.5f, pero los parametros de esta"
                  " version estan calibrados para punto=0.01 (Axi, 2 digitos)."
                  " Revisa SL/trailing/min range antes de operar.",
                  _Symbol, _Point);

   PrintFormat("IFVG_DualModel_EA_Axi_Live iniciado. Modo=%s Simbolo=%s"
               " BalanceRef=%.2f", EnumToString(InpModelMode), _Symbol,
               g_initialBalance);
...
void UpdatePanel()
  {
   ...
   string txt = StringFormat(
      "IFVG DualModel EA AXI LIVE | Modo: %s\n"
